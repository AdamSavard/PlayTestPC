...
public class MainView extends JRibbonFrame {
	...
	private class CardKeyListener implements KeyListener {
		public void keyPressed(KeyEvent e) {
			...
			if(e.getKeyCode() == KeyEvent.VK_ENTER){ // If enter
				...
				if(isFinalRow(txt)){ // If final row, move down to next component
		            if(e.getModifiers() > 0){
		            	e.getComponent().transferFocusBackward();
		            }else{
		            	e.getComponent().transferFocus();
				...
				// If there are already enough line seperators
				if(txt.getText().split("\r\n|\r|\n", -1).length >= txt.getRows()){
					e.consume();
					return; // Do nothing
				}
				//Add line seperater properly
	...
	private class CommentChanged implements DocumentListener {
		...
		public void update(DocumentEvent e){ // Card comment changed
			if(componentLock > 0){
				componentLock--;
				return;
			}
			...
			if(owner == null) return;
			controller.updateComments(CARD_EDITOR_LIST, commentText.getText());
		}
	}
	...
	public void buildLabelsAndTexts(String templateName, List<CardComponent> comps){
		...
		commentText = new JTextArea(5, columns);
		commentText.addKeyListener(new CardKeyListener());
		commentText.getDocument().addDocumentListener(new CommentChanged());
	...
	public void updateCardComments(String newComments){
		if(commentText.getText().equals(newComments)) return;
		componentLock++;
		commentText.setText(newComments);
	}
